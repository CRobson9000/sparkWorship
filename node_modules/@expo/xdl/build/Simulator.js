...
async function installExpoOnSimulatorAsync({
  ...
  if (version) {
    _Logger().default.global.info(`Installing Expo Go ${version} on ${simulator.name}`);
  } else {
    _Logger().default.global.info(`Installing Expo Go on ${simulator.name}`);
  }
...
async function uninstallExpoAppFromSimulatorAsync({
  ...
  try {
    _Logger().default.global.info('Uninstalling Expo Go from iOS simulator.');
