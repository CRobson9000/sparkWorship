...
async function installExpoAsync({
  ...
  if (version) {
    _Logger().default.global.info(`Installing Expo Go ${version} on device`);
  } else {
    _Logger().default.global.info(`Installing Expo Go on device`);
  }
...
async function uninstallExpoAsync(device) {
  _Logger().default.global.info('Uninstalling Expo Go from Android device.'); // we need to check if its installed, else we might bump into "Failure [DELETE_FAILED_INTERNAL_ERROR]"
  ...
  try {
    ...
    _Logger().default.global.error('Could not uninstall Expo Go from your device, please uninstall Expo Go manually and try again.');
